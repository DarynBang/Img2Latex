...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        return f + identity  # Skip connection
...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Returns:
            (B, d_model, H, W)
        """
        assert x.shape[2] == self.pos_encoder.shape[2], f"Input d_model ({x.shape[2]}) does not match positional encoding d_model ({self.pos_encoder.shape[2]})"  # type: ignore
...
class ResNet34_Transformer(nn.Module):
    def __init__(self,
                 d_model: int,
                 dim_feedforward: int,
                 nhead: int,
