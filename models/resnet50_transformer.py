...
class PositionalEncoding1D(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 300) -> None:
        ...
        pe = self.make_pe(d_model, max_len)  # (d_model, 1, max_len)
...
class ResNet50_Transformer(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        for layer in self.bottleneck:
            if isinstance(layer, nn.Conv2d):  # Only initialize Conv layers
                nn.init.kaiming_normal_(
                    layer.weight, a=0, mode="fan_out", nonlinearity="leaky_relu"
                )
